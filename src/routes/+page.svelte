<script lang="ts">
	...
	async function sendOtp(username: string) {
		...
		console.log(`${username}@up.edu.ph`);
...
<div class="flex h-screen w-screen flex-col">
	...
	<section class="flex grow flex-col items-center">
		...
		<button
			on:click={() => {
				loginRfid(rfid);
			}}>Login with RFID</button
		>

		<!-- Send OTP -->
		<input type="text" class="border border-4" bind:value={username} />
		<button
			on:click={() => {
				sendOtp(username);
			}}>Send OTP to UP Mail</button
		>

		<!-- Login with OTP -->
		<input type="text" class="border border-4" bind:value={otp} />
		<button
			on:click={() => {
				loginOtp(otp);
